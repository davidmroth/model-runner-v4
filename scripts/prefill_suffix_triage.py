...
def main() -> int:
    ...
    for line in lines:
        if "[pc] lookup miss" in line:
            ...
            events.append(("miss", m.group(1) if m else "?", line))
    ...
    for kind, scope, _line in events:
        if kind == "commit":
    ...
    print(f"lookup_miss total: {sum(1 for k, _, _ in events if k == 'miss')}")
    ...
    print(f"commits: {sum(1 for k,_,_ in events if k=='commit')} "
          f"unique={len({s for k,s,_ in events if k=='commit'})}")
    print(f"hits: {sum(1 for k,_,_ in events if k=='hit')} "
          f"unique={len({s for k,s,_ in events if k=='hit'})}")
    print(f"scope_evict lines: {sum(1 for k,_,_ in events if k=='scope_evict')}")
    print(f"cut_evict lines: {sum(1 for k,_,_ in events if k=='cut_evict')}")
    print(f"deferred snap failed: {sum(1 for k,_,_ in events if k=='def_fail')}")
